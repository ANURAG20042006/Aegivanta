...
async def prometheus_metrics(db: AsyncSession = Depends(get_db)):
    ...
    redis_conn = 1 if dist_m.get("is_connected") else 0

    lines = [
        "# HELP aegivanta_uptime_seconds Application uptime in seconds",
        "# TYPE aegivanta_uptime_seconds gauge",
        f"aegivanta_uptime_seconds {uptime_sec}",
        f"sentinel_uptime_seconds {uptime_sec}",
        "",
        "# HELP aegivanta_database_healthy Database connectivity status (1 = healthy, 0 = unhealthy)",
        "# TYPE aegivanta_database_healthy gauge",
        f"aegivanta_database_healthy {db_ok}",
        f"sentinel_database_healthy {db_ok}",
        "",
        "# HELP aegivanta_database_latency_ms Database roundtrip latency in milliseconds",
        "# TYPE aegivanta_database_latency_ms gauge",
        f"aegivanta_database_latency_ms {db_latency_ms}",
        f"sentinel_database_latency_ms {db_latency_ms}",
        "",
        "# HELP aegivanta_redis_connected Distributed Redis streaming broker connection status",
        "# TYPE aegivanta_redis_connected gauge",
        f"aegivanta_redis_connected {redis_conn}",
        f"sentinel_redis_connected {redis_conn}",
        "",
        "# HELP aegivanta_stream_ingested_total Total streaming telemetry events ingested",
        "# TYPE aegivanta_stream_ingested_total counter",
        f"aegivanta_stream_ingested_total {stream_m['total_ingested'] + dist_m['published_total']}",
        f"sentinel_stream_ingested_total {stream_m['total_ingested'] + dist_m['published_total']}",
        "",
        "# HELP aegivanta_stream_published_total Total telemetry events published to durable stream",
        "# TYPE aegivanta_stream_published_total counter",
        f"aegivanta_stream_published_total {dist_m['published_total']}",
        f"sentinel_stream_published_total {dist_m['published_total']}",
        "",
        "# HELP aegivanta_stream_consumed_total Total streaming telemetry events consumed by workers",
        "# TYPE aegivanta_stream_consumed_total counter",
        f"aegivanta_stream_consumed_total {dist_m['consumed_total']}",
        f"sentinel_stream_consumed_total {dist_m['consumed_total']}",
        "",
        "# HELP aegivanta_stream_acked_total Total stream messages successfully processed and acknowledged",
        "# TYPE aegivanta_stream_acked_total counter",
        f"aegivanta_stream_acked_total {dist_m['acked_total']}",
        f"sentinel_stream_acked_total {dist_m['acked_total']}",
        "",
        "# HELP aegivanta_stream_retried_total Total stream message retry attempts",
        "# TYPE aegivanta_stream_retried_total counter",
        f"aegivanta_stream_retried_total {dist_m['retried_total']}",
        f"sentinel_stream_retried_total {dist_m['retried_total']}",
        "",
        "# HELP aegivanta_stream_duplicates_total Total duplicate telemetry events rejected by deduplication gate",
        "# TYPE aegivanta_stream_duplicates_total counter",
        f"aegivanta_stream_duplicates_total {stream_m['total_duplicates'] + dist_m['duplicate_total']}",
        f"sentinel_stream_duplicates_total {stream_m['total_duplicates'] + dist_m['duplicate_total']}",
        "",
        "# HELP aegivanta_stream_dlq_total Total events routed to Dead Letter Queue",
        "# TYPE aegivanta_stream_dlq_total counter",
        f"aegivanta_stream_dlq_total {stream_m['total_dlq'] + dist_m['dlq_total']}",
        f"sentinel_stream_dlq_total {stream_m['total_dlq'] + dist_m['dlq_total']}",
        "",
        "# HELP aegivanta_stream_dlq_depth Current depth of in-memory Dead Letter Queue",
        "# TYPE aegivanta_stream_dlq_depth gauge",
        f"aegivanta_stream_dlq_depth {stream_m['dlq_depth']}",
        f"sentinel_stream_dlq_depth {stream_m['dlq_depth']}",
        "",
        "# HELP aegivanta_pubsub_published_total Total messages published to Redis Pub/Sub backplane",
        "# TYPE aegivanta_pubsub_published_total counter",
        f"aegivanta_pubsub_published_total {dist_m['pubsub_published_total']}",
        f"sentinel_pubsub_published_total {dist_m['pubsub_published_total']}",
        "",
        "# HELP aegivanta_websocket_broadcast_total Total WebSocket events broadcasted across distributed instances",
        "# TYPE aegivanta_websocket_broadcast_total counter",
        f"aegivanta_websocket_broadcast_total {dist_m['websocket_broadcast_total']}",
        f"sentinel_websocket_broadcast_total {dist_m['websocket_broadcast_total']}",
        "",
        "# HELP aegivanta_pcap_files_processed_total Total PCAP binary files ingested and processed",
        "# TYPE aegivanta_pcap_files_processed_total counter",
        f"aegivanta_pcap_files_processed_total {PCAPTelemetryService.pcap_files_processed}",
        f"sentinel_pcap_files_processed_total {PCAPTelemetryService.pcap_files_processed}",
        "",
        "# HELP aegivanta_pcap_packets_parsed_total Total network packet frames parsed from PCAP",
        "# TYPE aegivanta_pcap_packets_parsed_total counter",
        f"aegivanta_pcap_packets_parsed_total {PCAPTelemetryService.pcap_packets_parsed}",
        f"sentinel_pcap_packets_parsed_total {PCAPTelemetryService.pcap_packets_parsed}",
        "",
        "# HELP aegivanta_pcap_flows_extracted_total Total 5-tuple bidirectional network flows extracted from PCAP",
        "# TYPE aegivanta_pcap_flows_extracted_total counter",
        f"aegivanta_pcap_flows_extracted_total {PCAPTelemetryService.pcap_flows_extracted}",
        f"sentinel_pcap_flows_extracted_total {PCAPTelemetryService.pcap_flows_extracted}",
        "",
        "# HELP aegivanta_pcap_parse_errors_total Total PCAP parsing or validation errors",
        "# TYPE aegivanta_pcap_parse_errors_total counter",
        f"aegivanta_pcap_parse_errors_total {PCAPTelemetryService.pcap_parse_errors}",
